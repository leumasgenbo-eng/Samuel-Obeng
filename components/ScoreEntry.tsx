...
import { generateSubjectRemark, getObservationRating } from '../utils';
...
const ScoreEntry: React.FC<ScoreEntryProps> = ({ students, setStudents, settings, onSettingChange, onSave, department, schoolClass, subjectList }) => {
  ...
  const calculateHybridScore = (scoreA: number, scoreB: number) => {
      if (isEarlyChildhood) {
          // For Early Childhood, we assume A and B are both on 100 scale.
          // Final score is average of the two (50/50)
          const final = (scoreA + scoreB) / 2;
      ...
      if (isScience && isJHS) {
          ...
          total = Math.round(total / 1.4); // Legacy scaling if max exceeds 100
      } else {
  ...
  const handleScoreChange = (id: number, field: 'sectionA' | 'sectionB', value: string) => {
    ...
    // Daycare Logic: Manual Entry Allowed for both A and B
    if (isEarlyChildhood) {
        ...
        setStudents(prev => prev.map(student => {
            ...
            // Recalculate total based on new input
    ...
    if (field === 'sectionA') {
        if (numValue > 100) numValue = 100; // Relax max to allow higher raw scores if manually entered
        if (numValue < 0) numValue = 0;
    }
    if (field === 'sectionB') {
        const maxB = (isScience && isJHS) ? 100 : 100; // Relax max 
  ...
  // --- Auto-Compute Logic (Batch) ---
  const handleComputeScores = () => {
      const sbaConfig = settings.sbaConfig?.[selectedSubject] || { cat1Max: 15, cat2Max: 15, cat3Max: 10 };
      const sbaWeight = sbaConfig.cat1Max + sbaConfig.cat2Max + sbaConfig.cat3Max;
      
      const confirmMsg = isEarlyChildhood 
        ? `This will auto-compute scores for ${selectedSubject}:\n1. Average Daily Score (from Daily Grid) -> Entry A\n2. Exam Score (from Observations) -> Entry B\n\nContinue?`
        : `This will auto-compute scores for ${selectedSubject}:\n1. SBA Score (Sum of CAT 1, 2, 3) -> Section A\n\nContinue?`;
      ...
      if (window.confirm(confirmMsg)) {
          ...
          setStudents(prev => prev.map(student => {
              ...
              // 1. Calculate SBA / Average Daily Score from Grid
              let computedSBA = 0;
              const dailyScores = student.assessmentScores?.[selectedSubject];
              
              if (isEarlyChildhood) {
                  // Early Childhood Logic (Generic Columns Average)
                  if (dailyScores && columns.length > 0) {
                       ...
                       if (totalMax > 0) {
                           computedSBA = (totalObtained / totalMax) * 100; // Scale to 100 for Entry A
                       }
                  } else {
                       computedSBA = student.scoreDetails?.[selectedSubject]?.sectionA || 0;
                  }
              } else {
                  // Basic/JHS Logic (CAT 1, 2, 3 Summation)
                  if (dailyScores) {
                      const c1 = dailyScores['cat1'] || 0;
                      const c2 = dailyScores['cat2'] || 0;
                      const c3 = dailyScores['cat3'] || 0;
                      computedSBA = c1 + c2 + c3;
                  } else {
                      // If no data, keep existing or 0
                      computedSBA = student.scoreDetails?.[selectedSubject]?.sectionA || 0;
                  }
              }
              
              const finalA = Math.round(computedSBA);
              
              // 2. Entry B / Exam Score Logic
              let finalB = 0;
              if (isEarlyChildhood) {
                  // Early Childhood uses Observations for B
                  const obsScores = student.observationScores?.[selectedSubject] || [];
                  if (obsScores.length > 0) {
                      const sum = obsScores.reduce((a: number, b: number) => a + b, 0);
                      const avg = sum / obsScores.length;
                      finalB = Math.round((avg / 9) * 100); 
                  } else {
                      finalB = student.scoreDetails?.[selectedSubject]?.sectionB || 0;
                  }
              } else {
                  // Basic/JHS preserves manual Section B (Exam Score)
                  finalB = student.scoreDetails?.[selectedSubject]?.sectionB || 0;
              }

              const finalTotal = calculateHybridScore(finalA, finalB);

              const newDetails = { ...student.scoreDetails?.[selectedSubject], sectionA: finalA, sectionB: finalB, total: finalTotal };
              
              return {
                  ...student,
                  scores: { ...student.scores, [selectedSubject]: finalTotal },
                  scoreDetails: { ...student.scoreDetails, [selectedSubject]: newDetails }
              };
          }));
          alert("Scores computed and updated!");
      }
  };
  ...
  return (
    <div className="bg-white p-6 rounded shadow-md max-w-6xl mx-auto min-h-screen pb-96">
      ...
      <div className="flex flex-col gap-4 mb-6 bg-gray-50 p-4 rounded border border-gray-200">
        ...
        {/* Grading Logic Panel */}
        {(!isIndicator || !isEarlyChildhood) && (
            <div className="bg-white border border-blue-200 rounded p-4 shadow-sm flex flex-col md:flex-row gap-6 items-center">
                <div className="flex-1">
                    <h4 className="text-xs font-bold uppercase text-blue-900 mb-2 border-b pb-1">Grading Configuration</h4>
                    <p className="text-xs text-gray-600">
                        Use <strong>Auto-Compute</strong> to pull data from modules:
                        <br/>
                        1. {isEarlyChildhood ? <strong>Daily Assessment Grid</strong> : <strong>SBA Module</strong>} scores -> {isEarlyChildhood ? 'Entry A' : 'Section A'}.
                        {isEarlyChildhood && <><br/>2. <strong>Observation Points</strong> -> Entry B.</>}
                    </p>
                </div>
                <div className="border-l pl-4">
                    <button 
                        onClick={handleComputeScores}
                        className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded shadow font-bold text-sm flex items-center gap-2"
                        title="Auto-fill scores from linked modules"
                    >
          ...
          <thead className="bg-gray-100">
            <tr>
              ...
              {isEarlyChildhood && !isIndicator ? (
                 <>
                    <th className="border p-2 w-48 text-center bg-blue-50 text-blue-900 text-xs">
                        Average Daily Score<br/>(Average Indicator Point %)
                    </th>
                    <th className="border p-2 w-48 text-center bg-blue-50 text-blue-900 text-xs">
                        Exam Score<br/>(Observation Score)
                    </th>
                    <th className="border p-2 w-24 text-center bg-green-50 text-green-900 font-bold">Total Score</th>
                 </>
              ) : (
                <>
                  {!isEarlyChildhood && (
                      <th className="border p-2 w-24 text-center bg-blue-50">
                          Sec A (SBA)
                          <div className="text-[10px] font-normal text-gray-500">(Auto-fill)</div>
                      </th>
                  )}
                  <th className="border p-2 w-24 text-center bg-blue-50">
                    {isIndicator ? 'Rating' : 'Sec B (Exam)'}
                  </th>
                </>
              )}
              ...
              <th className="border p-2 w-20 text-center">Act (Remove)</th>
            </tr>
          </thead>
          <tbody>
            {students.map(student => {
              ...
              if (isIndicator) {
                  const obsScores = student.observationScores?.[selectedSubject];
                  if (obsScores && obsScores.length > 0) {
                      derivedRating = getObservationRating(obsScores);
                      const avg = obsScores.reduce((a,b)=>a+b,0) / obsScores.length;
                      obsAvgDisplay = avg.toFixed(1);
                  }
              }
              ...
              return (
                ...
                >
                  ...
                  {isEarlyChildhood && !isIndicator ? (
                      <>
                        ...
                        <td className="border p-2 text-center bg-blue-50/50">
                         ...
                         <td className="border p-2 text-center bg-green-50 font-bold text-green-800 text-lg">
                            {displayTotal}
                        </td>
                      </>
                  ) : (
                      <>
                        {!isEarlyChildhood && (
                            <td className="border p-2 text-center bg-blue-50/50">
                                <input type="number" min="0" value={valA === 0 ? '' : valA} onChange={(e) => handleScoreChange(student.id, 'sectionA', e.target.value)} className="w-full text-center p-1 border rounded font-bold text-blue-900" onClick={(e) => e.stopPropagation()} placeholder="SBA"/>
                            </td>
                        )}
                        <td className="border p-2 text-center bg-blue-50/50">
                            {isIndicator ? (
                                ...
                                <input 
                                    ...
                                    value={valB === 0 ? '' : valB} 
                                    onChange={(e) => handleScoreChange(student.id, 'sectionB', e.target.value)} 
                                    className="w-full text-center p-1 border rounded font-bold text-blue-900" 
                                    onClick={(e) => e.stopPropagation()}
                                    placeholder="Exam"
                                />
                            )}
                  ...
                  {!isEarlyChildhood && <td className="border p-2 text-center font-bold bg-gray-100 text-lg">{rawTotal}</td>}
