...
const App: React.FC = () => {
  ...
  useEffect(() => {
     ...
     if (availableClasses && availableClasses.length > 0) {
         ...
         setActiveStream(""); // Reset stream on dept change
     }
  }, [activeDept]);
  ...
  // Compute Effective Class Name (with Stream)
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans flex flex-col">
      ...
      {/* 2. Second Level: Class Navigation & Stream Toggle */}
      ...
      {isExamDept && activeModule === 'Assessment' && (
          <div className="no-print bg-gray-50 border-b border-gray-200 px-4 py-2 flex gap-4 justify-center flex-wrap">
             ...
             >
                {isEarlyChildhood ? 'Learning Area / Subject' : 'Subject List'}
             </button>
             <button
                onClick={() => setExamSubTab('daily_assessment')}
                className={`pb-1 px-4 font-bold text-sm border-b-2 transition-colors ${examSubTab === 'daily_assessment' ? 'border-blue-600 text-blue-900' : 'border-transparent text-gray-500 hover:text-blue-600'}`}
             >
      ...
      {/* 4b. Sub-Module Navigation for Time Table */}
